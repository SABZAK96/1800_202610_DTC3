import"./site-footer-C38TklsK.js";import{o as T}from"./authentication-ByHJCXUm.js";import{o as j,a as p,b as m,d,s as Z,e as x,c as y,g as M,u as b,f as q,h as N,i as O,j as E}from"./firebaseConfig-C9Uf5pfZ.js";const R=document.getElementById("username-display"),L=document.getElementById("auth-btn"),_=document.querySelector(".signedinuser");j(x,async e=>{if(_&&_.classList.remove("hidden"),e){R.textContent=e.displayName||e.email||"User";const t=await p(m(d,"users",e.uid)),n=t.data();t.exists()&&n.profileImage&&(document.getElementById("profile-img").src="data:image/png;base64,"+n.profileImage),L.onclick=async()=>{try{await Z(x),window.location.href="index.html"}catch{console.log("Logout error")}}}else L.onclick=()=>{window.location.href="login.html"}});function U(){const e=document.getElementById("username-display");T(t=>{if(t)document.getElementById("unsigneduser").classList.toggle("hidden"),C(t);else{location.href="index.html";return}const n=t.displayName||t.email;e&&(e.textContent=`${n}!`)})}U();let g=[];async function k(e,t,n,r){const i=document.querySelector(".favourites-container");if(t){async function v(){const a=(await p(m(d,"users",t.uid))).data();if(a.favorite_events)return a.favorite_events;{let l=[];return await O(m(d,"users",t.uid),{favorite_events:l},{merge:!0}),l}}async function f(s){await v(),(await p(m(d,"users",t.uid))).data().favorite_events.includes(n)?(s.style.fill="red",s.style.stroke="none"):(s.style.fill="black",s.style.stroke="black")}f(e),e.parentElement.addEventListener("click",function(){u(e)});async function u(s){await v();let a=!1;if((await p(m(d,"users",t.uid))).data().favorite_events.includes(n)?a=!0:a=!1,a==!1)s.style.fill="red",s.style.stroke="none",a=!0,await b(m(d,"users",t.uid),{favorite_events:q(n)});else if(a=!1,s.style.fill="black",s.style.stroke="black",await b(m(d,"users",t.uid),{favorite_events:N(n)}),r.remove(),g.splice(g.indexOf(n),1),C(t),i.querySelectorAll(".favbtn").length===0){let c='<div class="col-span-full flex flex-col items-center justify-center gap-4 py-10 w-full"><p>No favorited events yet!</p><a href="explore.html" class="bg-black h-10 px-6 rounded-full text-white text-sm flex items-center justify-center">Go Explore</a></div>';const h=document.createElement("div");h.innerHTML=c,i.appendChild(h.firstElementChild)}return s}}else e.parentElement.addEventListener("click",function(){window.location.href="login.html"})}async function C(e){const t=y(d,"Events_2026"),n=await M(t),r=await p(m(d,"users",e.uid)),i=r.exists()?r.data():{},v=i.favorite_events||[];let f=0;const u=document.querySelector(".favourites-container");if(u&&(u.innerHTML="",g=[]),n.forEach(s=>{let a=s.data(),l=s.id;if(v.includes(l)&&f<3){let o=`
