...
// Fetches events from Firestore and populates the index page.
// The first event becomes the large featured card.
// The next 5 events fill the "This Week" grid below it.
async function loadindex() {
  ...
  snap.forEach((doc) => {
    ...
    // First event → large featured card at the top
    if (populate === 0) {
      let result = `
    ...
    // Events 2-6 → smaller grid cards in the "This Week" section
      ...
      // Heart button redirects to login since index page is for unauthenticated users
...
// Constructs the URL with a "search" query param so explore.js
// can pre-fill the search box and filter results automatically.
